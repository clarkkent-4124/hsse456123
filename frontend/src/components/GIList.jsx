...
const BADGE = {
  PICKUP: { colorVar: 'var(--pickup)', bgVar: 'var(--pickup-bg)', borderHex: '#ef444433' },
  RNR:    { colorVar: 'var(--rnr)',    bgVar: 'var(--rnr-bg)',    borderHex: '#3b82f633' },
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
        {TYPE_CHIPS.map(chip => {
          ...
          const colorVar = chip === 'Semua' ? 'var(--accent)'
            : chip === 'PICKUP' ? 'var(--pickup)'
            : chip === 'RNR'    ? 'var(--rnr)'
            : 'var(--tcs)';
          const bgVar = chip === 'Semua' ? 'var(--accent-bg)'
            : chip === 'PICKUP' ? 'var(--pickup-bg)'
            : chip === 'RNR'    ? 'var(--rnr-bg)'
            : 'var(--tcs-bg)';
          return (
            <button
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
        {(loading || applying) ? (
          ...
          <div
            key={gi.gi_name}
            onClick={() => onGIClick && onGIClick(gi.gi_name)}
            style={{
              padding: '12px 14px',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{gi.gi_name}</div>
                <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 1 }}>
                  {parseInt(gi.total)} alarm aktif
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              {['PICKUP', 'RNR', 'TCS'].map(type => parseInt(gi[type]) > 0 && (
                <span key={type} style={{
